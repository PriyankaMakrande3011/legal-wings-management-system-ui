...
const AssignLead = ({ isOpen, onClose, onAssignSuccess, leadId }) => {
  ...
  const fetchExecutives = async () => {
    try {
      ...
      const response = await axios.get(
        "http://legalwingcrm.in:8081/legal-wings-management/users/dropDown?userType=EXECUTIVE",
        {
          headers: {
  ...
  const handleAssign = async (executive) => {
    ...
    try {
      await axios.put(
        `http://legalwingcrm.in:8081/legal-wings-management/leads/${leadId}/assign?userId=${userId}`,
        {},
        {
          headers: {
            "Authorization": `Bearer ${keycloak.token}`,
